...
async function processOpenAiRing({
  ...
  try {
    ...
    const elevenLabsApiKey =
      "sk_83edfc4cde24c7a4a9813511cac7548894d43f8a85bedfb5";
    const voiceId = "C3Xl8XfD9Miedb6pGEKK";
    const tempPath = `${__dirname}/../client/public/temp`;

    const elevLabsAudio = await getAiAudioResponse({
      convoPath: convoPath,
      historyLength: node?.data?.history,
      trainText: node?.data?.trainText,
      openaiApiKey: node?.data?.keys,
    ...
    console.log(JSON.stringify({ elevLabsAudio }));
    ...
    // const elevLabsAudio = await getElevenLabsAudio({
    ...
    if (!elevLabsAudio?.success) {
      console.error("Error in processOpenAiRing:", elevLabsAudio);
      hangupCall({
        msg:
          elevLabsAudio?.err ||
          elevLabsAudio?.msg ||
          "Problem found in Voice API or OpenAI",
        twiml,
        res,
      });
    } else if (elevLabsAudio?.function) {
      console.log("FUNCTION FOUND");

      const redirectUrl = outgoingLog?.id
        ? `/api/ivr/reply?device=${device}&id=${elevLabsAudio?.function}&mysql=${mysql}&outgoing=${outgoingLog?.id}`
        : `/api/ivr/reply?device=${device}&id=${elevLabsAudio?.function}&mysql=${mysql}`;
      twiml.redirect(redirectUrl);
    } else {
      // const pauseSec = estimateSpeakingTime(
      //   replyByAi?.msg,
      ...
      twiml.play(elevLabsAudio?.url);

      twiml.pause({ length: elevLabsAudio?.duration });

      console.log(`Waiting for ${elevLabsAudio?.duration} sec`);

      const newObj = {
        role: "assistant",
        content: elevLabsAudio?.msg,
      };
