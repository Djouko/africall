...
async function processSay({
  ...
  try {
    ...
    const platform = node.data?.voice;

    const twiml = new VoiceResponse();

    if (platform === "ELEVENLABS") {
      const elevenLabsApiKey = node.data?.elevenLabsData?.apiKey;
      const voiceId = node.data?.elevenLabsData?.voiceId;

      const tempPath = `${__dirname}/../client/public/temp`;

      const result = await getElevenLabsAudio({
      ...
      if (result?.success && result?.url) {
        twiml.play(result.url);
        if (result?.duration) {
          twiml.pause({ length: Math.ceil(result.duration) });
        }
      } else {
        // Fallback in case ElevenLabs failed
        twiml.say(
          {
      ...
      // Default Twilio voice (Polly etc.)
      twiml.say(
        {
          language: node.data?.local?.language?.locale,
          voice: `Polly.${node.data?.local?.voice?.name}`,
    ...
    console.error("Error in processSay:", err);
...
async function processCustomAudio({
  ...
  try {
    // console.log("Came to custom audio");

    const twiml = new VoiceResponse();
    const audioUrl = node?.data?.audioUrl;
    const audioDuration = node?.data?.duration;

    // console.log({ audioUrl, audioDuration: Math.ceil(audioDuration) });
    twiml.play(audioUrl);
    twiml.pause({ length: Math.ceil(audioDuration) });
    ...
    console.error("Error in processCustomAudio:", err);
...
//     console.log({
//       lulli: JSON.stringify({ node, objVar, device, mysql, outgoingLog }),
//     });
...
async function processOpenAiRing({
  ...
  try {
    ...
    if (!replyByAi?.success) {
      ...
      hangupCall({
        msg: replyByAi?.err || "Could not fetch reply from OpenAI",
        twiml,
        res,
      });
      ...
      if (platform === "ELEVENLABS") {
        ...
        // console.log({ elevenLabsResult: result });

        if (result?.success && result?.url) {
          twiml.play(result.url);
          if (result?.duration) {
            // console.log(`twiml paused for ${result.duration} sec}`);
            twiml.pause({ length: Math.ceil(result.duration) });
          }
        } else {
          // Fallback to native Polly
          twiml.say(
            {
              language: node.data?.local?.language?.locale,
              voice: `Polly.${node.data?.local?.voice?.name}`,
    ...
    console.error("Error in processOpenAiRing:", err);
    const twiml = new VoiceResponse();
    moveOutgoingNext({
      logId: outgoingLog?.id,
      hangup: true,
...
async function processOpenAi({
  ...
  try {
    ...
    if (ring) {
      ...
      twiml.play(
        process.env.BEEPSOURCE ||
          "https://www.soundjay.com/buttons/beep-07a.mp3"
        // "https://www.soundjay.com/buttons/beep-02.wav"
      );

      // console.log(`twiml paused for 0.5 sec}`);
    ...
    console.error("Error in processOpenAi:", err);
    const twiml = new VoiceResponse();
    moveOutgoingNext({
      logId: outgoingLog?.id,
      hangup: true,
...
async function processGather({
  ...
  try {
    ...
    console.error("Error in processGather:", err);
