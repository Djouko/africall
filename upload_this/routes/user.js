...
router.post("/save_google_credentials", validateUser, async (req, res) => {
  try {
    const { credential_id, service_account_json } = req.body;

    if (!credential_id || !service_account_json) {
    ...
    try {
      ...
      return res.json({ success: false, msg: "Invalid JSON format" });
    }
    ...
    if (
      ...
      return res.json({
        success: false,
        msg: "Invalid service account JSON structure",
      });
    }

    // Check if credential_id already exists for this user
    const existing = await query(
      `SELECT id FROM google_credentials WHERE uid = ? AND credential_id = ?`,
      [req.decode.uid, credential_id]
    );
    ...
    if (existing.length > 0) {
      ...
      // Insert new
      await query(
        `INSERT INTO google_credentials (uid, credential_id, service_account_json, project_id, client_email) 
         VALUES (?, ?, ?, ?, ?)`,
        [
          req.decode.uid,
          credential_id,
    ...
    res.json({ success: true, msg: "Google credentials saved successfully" });
  } catch (err) {
...
router.get("/get_google_credentials", validateUser, async (req, res) => {
  try {
    const data = await query(
      `SELECT id, credential_id, project_id, client_email, is_active, created_at 
       FROM google_credentials WHERE uid = ? AND is_active = 1`,
      [req.decode.uid]
    );
...
router.delete(
  "/delete_google_credentials/:credential_id",
  validateUser,
  async (req, res) => {
    try {
      ...
      await query(
        `UPDATE google_credentials SET is_active = 0 WHERE uid = ? AND credential_id = ?`,
        [req.decode.uid, credential_id]
      );

      res.json({
        success: true,
        msg: "Google credentials deleted successfully",
      });
    } catch (err) {
      res.json({
        success: false,
...
router.post("/call_logs_device", validateUser, async (req, res) => {
  try {
    const { sid, token, twilioNumber, limit = 15, offset = 0 } = req.body;

    if (!sid || !token || !twilioNumber) {
      return res.json({
        msg: "Invalid request or enter your device API properly ",
        success: false,
      });
    }

    const logsData = await getCallLogsForNumber({
      sid,
      token,
      twilioNumber,
      limit,
...
router.post("/return_url", validateUser, async (req, res) => {
  try {
    ...
    const inputTempPath = path.join(
      __dirname,
      "../temp",
      `${randomString}_original.mp3`
    );
    const finalFilename = `${randomString}.mp3`;
    ...
    await file.mv(inputTempPath);

    // Convert to standard mp3: 128kbps bitrate, 44.1kHz sample rate
    ffmpeg(inputTempPath)
      ...
      .on("end", () => {
        ...
        const url = `${process.env.FRONTENDURI}/media/${finalFilename}`;
        res.json({
          success: true,
          url,
      ...
      .on("error", (err) => {
        ...
        res
          .status(500)
          .json({ success: false, msg: "FFmpeg conversion failed", err });
      })
      .save(outputPath);
  } catch (err) {
    console.log(err);
