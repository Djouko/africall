...
router.post("/add_force_task", validateUser, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      msg: `the work force task was added with ${getContact?.length} numbers`,
    });
  } catch (err) {
...
// updating call sid call logs
router.post("/update_call_sid", validateAgent, async (req, res) => {
  try {
    const { id, sid } = req.body;

    if (sid?.length > 5) {
      await query(`UPDATE call_force_log SET sid = ? WHERE id = ?`, [sid, id]);
    }
...
// get call recording details using CALL SID
router.post("/get_recording_details", validateUser, async (req, res) => {
  try {
    const { callSid, number } = req.body;

    const [device] = await query(
      `SELECT * FROM device WHERE number = ? AND uid = ?`,
      [number?.replace("+", ""), req.decode.uid]
    );

    if (!device) {
      return res.json({
        success: false,
        msg: "This device was either removed or not found to fetch the recording details",
      });
    }

    const client = twilio(device.sid, device.token);

    // List recordings for this Call SID
    ...
    if (!recordings.length) {
      return res.json({
        success: false,
        msg: "No recordings found for this call.",
      });
    }

    // Pick the first recording (or loop if there are multiple)
    const recording = recordings[0];

    // Fetch full recording details (includes mediaUrl)
    const fullDetails = await client.recordings(recording.sid).fetch();

    console.log({ fullDetails });

    // Return the full recording details (using mediaUrl from fullDetails)
    res.json({
      success: true,
      data: fullDetails,
      sid: device.sid,
